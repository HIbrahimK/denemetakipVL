...
export default function StudyPlanSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!token) {
      toast({
        title: 'Hata',
        description: 'Oturum bilgisi bulunamadý. Lütfen tekrar giriþ yapýn.',
        variant: 'destructive',
      });
      setSaving(false);
      router.push('/login/school');
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage = 'Ayarlar kaydedilirken hata oluþtu';
        try {
        ...
        if (response.status === 404) {
          errorMessage = 'Okul bulunamadý. Lütfen sayfayý yenileyip tekrar deneyin.';
        } else if (response.status === 401 || response.status === 403) {
          errorMessage = 'Bu iþlem için yetkiniz yok. SCHOOL_ADMIN rolü gerekli.';
      ...
      toast({
        title: 'Baþarýlý',
        description: 'Ayarlar baþarýyla kaydedildi',
      });
  ...
  return (
    <div className="container mx-auto py-6 max-w-4xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Çalýþma Planý Ayarlarý</h1>
          <p className="text-muted-foreground mt-1">
            Otomatik temizleme ve diðer ayarlarý yönetin
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Eski aktif planlarý otomatik olarak temizleyin. Þablonlar hiçbir zaman silinmez.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Her ayýn 1. günü eski aktif planlarý otomatik olarak temizle
              </p>
            </div>
            <Switch
              id="autoCleanup"
          ...
          {autoCleanupEnabled && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Örnek: {cleanupMonthsToKeep} ay = Son {cleanupMonthsToKeep} ay içinde oluþturulan planlar korunur
              </p>
            </div>
          )}
          ...
          <div className="p-4 border border-blue-200 rounded-lg bg-blue-50">
            ...
            <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
              ...
              <li>Þablonlar hiçbir zaman silinmez</li>
              <li>Temizleme iþlemi her ayýn 1. günü gece yarýsý çalýþýr</li>
              <li>Temizlenen planlarýn atamalarý ve görevleri de silinir</li>
              <li>Öðrenci performans verileri korunur</li>
            </ul>
          </div>
