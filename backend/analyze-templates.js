...
async function analyzeTemplate(filePath, label) {
    if (!fs.existsSync(filePath)) {
        return `\n${label}: DOSYA BULUNAMADI (${filePath})\n`;
    }
...
async function run() {
    ...
    fullOutput += await analyzeTemplate('d:\\denemetakip\\lgsSablon.xlsx', 'LGS İŞLENMİŞ ŞABLON (xlsx)');
    fullOutput += await analyzeTemplate('d:\\denemetakip\\lgsSablon.xls', 'LGS İŞLENMİŞ ŞABLON (xls)');
