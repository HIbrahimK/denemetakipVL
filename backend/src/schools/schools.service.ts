...
export class SchoolsService {
  ...
  async getPromotionPreview(id: string) {
    ...
    const classes = grades.flatMap((grade) =>
      grade.classes.map((classItem) => ({
        ...
        gradeLevel: this.parseGradeLevel(grade.name),
        studentCount: classItem._count.students,
      })),
    );

    const promotableClasses = classes.filter(
      (classItem) =>
        classItem.gradeLevel !== null &&
        promotableLevels.has(classItem.gradeLevel),
    );
    const archiveClasses = classes.filter(
      (classItem) =>
        classItem.gradeLevel !== null &&
        archivedLevels.has(classItem.gradeLevel),
    );

    const archivedClassIds = archiveClasses.map((item) => item.id);
    const archivedGradeIds = Array.from(
      new Set(archiveClasses.map((item) => item.gradeId)),
    );

    const promotedStudentCount = promotableClasses.reduce(
      (total, item) => total + item.studentCount,
      0,
    );
    const archivedStudentCount = archiveClasses.reduce(
      (total, item) => total + item.studentCount,
      0,
    );

    let estimatedArchivedParentCount = 0;
    let estimatedDeletedMessageRecipientCount = 0;

    if (archivedClassIds.length > 0) {
      const archivedStudents = await this.prisma.student.findMany({
        where: {
          schoolId: id,
          classId: { in: archivedClassIds },
        },
        select: {
          id: true,
          userId: true,
      ...
      const archivedStudentIds = archivedStudents.map((student) => student.id);
      const archivedStudentUserIds = archivedStudents.map(
      ...
      const archivedStudentIdSet = new Set(archivedStudentIds);
      const parentUserIdsToArchive: string[] = [];
      ...
      if (parentIds.length > 0) {
        ...
        for (const parent of parents) {
          const hasAnyActiveStudent = parent.students.some(
            (student) => !archivedStudentIdSet.has(student.id),
          );
          if (!hasAnyActiveStudent && parent.user.isActive) {
            parentUserIdsToArchive.push(parent.user.id);
          }
        }
      }

      estimatedArchivedParentCount = parentUserIdsToArchive.length;

      if (archivedStudentUserIds.length + parentUserIdsToArchive.length > 0) {
        const recipientUserIds = Array.from(
          new Set([...archivedStudentUserIds, ...parentUserIdsToArchive]),
        );

        estimatedDeletedMessageRecipientCount =
          await this.prisma.messageRecipient.count({
            where: {
              recipientId: { in: recipientUserIds },
              message: { schoolId: id },
            },
          });
      }
    ...
    const messageTargetOrConditions: Array<
      | { targetGradeId: { in: string[] } }
      | { targetClassId: { in: string[] } }
    > = [];

    if (archivedGradeIds.length > 0) {
      messageTargetOrConditions.push({
        targetGradeId: { in: archivedGradeIds },
      });
    }
    if (archivedClassIds.length > 0) {
      messageTargetOrConditions.push({
        targetClassId: { in: archivedClassIds },
      });
    }

    const estimatedDeletedMessageCount =
      messageTargetOrConditions.length > 0
        ? await this.prisma.message.count({
            where: {
              schoolId: id,
              OR: messageTargetOrConditions,
            },
          })
        : 0;

    const mentorGroupCount = await this.prisma.mentorGroup.count({
      where: { schoolId: id },
    });

    const promotionByLevel = Array.from(
      promotableClasses.reduce((acc, classItem) => {
        const level = classItem.gradeLevel!;
        const current = acc.get(level) ?? { classCount: 0, studentCount: 0 };
        current.classCount += 1;
        current.studentCount += classItem.studentCount;
        acc.set(level, current);
        return acc;
      }, new Map<number, { classCount: number; studentCount: number }>()),
    )
      .sort((a, b) => a[0] - b[0])
      .map(([fromLevel, stats]) => ({
        fromLevel,
    ...
    return {
      promotableClassCount: promotableClasses.length,
      promotableStudentCount: promotedStudentCount,
      archiveClassCount: archiveClasses.length,
      archiveStudentCount: archivedStudentCount,
      estimatedArchivedParentCount,
      estimatedDeletedMessageCount,
      estimatedDeletedMessageRecipientCount,
      mentorGroupCount,
      promotionByLevel,
      archivedLevels: Array.from(archivedLevels),
      promotableLevels: Array.from(promotableLevels),
    };
  }
  ...
  async promoteGrades(id: string) {
    ...
    const archiveReason = 'GRADE_PROMOTION_ARCHIVE';
    const archiveSuffix = `${archivedAt.getFullYear()}-${String(
      archivedAt.getMonth() + 1,
    ).padStart(2, '0')}`;

    const promotionSummary = await this.prisma.$transaction(
      async (tx) => {
        ...
        const gradeByLevel = new Map<number, { id: string; name: string }>();
        for (const grade of grades) {
          const level = this.parseGradeLevel(grade.name);
          if (level !== null && !gradeByLevel.has(level)) {
            gradeByLevel.set(level, { id: grade.id, name: grade.name });
          }
        }
        ...
        let renamedArchivedClassCount = 0;
        for (const archiveClass of archiveClasses) {
          const archivedName = await this.buildUniqueArchivedClassName(
            tx,
            id,
            archiveClass.gradeId,
            archiveClass.name,
            archiveSuffix,
          );

          if (archivedName !== archiveClass.name) {
            await tx.class.update({
              where: { id: archiveClass.id },
              data: { name: archivedName },
            });
            renamedArchivedClassCount += 1;
          }
        }

        const promotedClasses = classes.filter(
          (classItem) =>
            classItem.gradeLevel !== null &&
            promotableLevels.has(classItem.gradeLevel),
        );

        const promotedTargetClassIds: string[] = [];
        let promotedStudentsCount = 0;
        let createdTargetClassCount = 0;

        for (const sourceClass of promotedClasses) {
          const sourceLevel = sourceClass.gradeLevel!;
          const targetLevel = sourceLevel + 1;
          const targetGrade = await this.ensureGradeByLevel(
            tx,
            id,
            targetLevel,
            gradeByLevel,
          ...
          if (!targetClass) {
            ...
            createdTargetClassCount += 1;
          }

          const moved = await tx.student.updateMany({
            where: { schoolId: id, classId: sourceClass.id },
            data: { classId: targetClass.id },
          });

          promotedStudentsCount += moved.count;
          promotedTargetClassIds.push(targetClass.id);
        }

        const archivedStudents =
          archivedClassIds.length > 0
        ...
        const archivedStudentIds = archivedStudents.map(
          (student) => student.id,
        );
        const archivedStudentUserIds = archivedStudents.map(
          (student) => student.userId,
        ...
        if (archivedStudentUserIds.length > 0) {
          await tx.user.updateMany({
            where: { id: { in: archivedStudentUserIds } },
            data: {
              isActive: false,
              archivedAt,
              archiveReason,
        ...
        if (parentUserIdsToArchive.length > 0) {
          await tx.user.updateMany({
            where: { id: { in: parentUserIdsToArchive } },
            data: {
              isActive: false,
              archivedAt,
              archiveReason,
        ...
        if (messageTargetOrConditions.length > 0) {
        ...
        const archivedRecipientUserIds = Array.from(
          new Set([...archivedStudentUserIds, ...parentUserIdsToArchive]),
        );

        let deletedMessageRecipientCount = 0;
        if (archivedRecipientUserIds.length > 0) {
          const deletedRecipients = await tx.messageRecipient.deleteMany({
            where: {
              recipientId: { in: archivedRecipientUserIds },
              message: { schoolId: id },
            },
          });
          deletedMessageRecipientCount = deletedRecipients.count;
        }
        ...
        const deletedPendingExamNotifications = await tx.examNotification.deleteMany({
          where: {
            isSent: false,
            exam: {
              schoolId: id,
        ...
        const deletedGroups = await tx.mentorGroup.deleteMany({
          where: { schoolId: id },
        });

        return {
          archivedStudentCount: archivedStudentIds.length,
          archivedParentCount: parentUserIdsToArchive.length,
          promotedStudentCount: promotedStudentsCount,
          promotedClassCount: promotedClasses.length,
          createdTargetClassCount,
          renamedArchivedClassCount,
          deletedGroupCount: deletedGroups.count,
          deletedMessageCount,
          deletedMessageRecipientCount,
          archivedExamCount: archivedExams.count,
          deletedPendingExamNotificationCount:
            deletedPendingExamNotifications.count,
          archivedGradeIds,
          archivedClassIds,
          promotedTargetClassIds: Array.from(new Set(promotedTargetClassIds)),
        };
      },
      {
        maxWait: 120000,
    ...
    return {
      message: 'Sinif atlatma ve arsivleme islemi tamamlandi',
      backup: {
        id: backupResult.backup.id,
        filename: backupResult.backup.filename,
        createdAt: backupResult.backup.createdAt,
  ...
  private parseGradeLevel(gradeName: string): number | null {
  ...
  private async ensureGradeByLevel(
    tx: any,
    schoolId: string,
    level: number,
    gradeByLevel: Map<number, { id: string; name: string }>,
  ) {
    const existing = gradeByLevel.get(level);
    if (existing) {
      return existing;
    }
    ...
    gradeByLevel.set(level, created);
    return created;
  }

  private async buildUniqueArchivedClassName(
    tx: any,
    schoolId: string,
    gradeId: string,
    originalName: string,
    ...
    const baseName = `${originalName}-ARSIV-${archiveSuffix}`;
    ...
    try {
      ...
      const backupContent = {
        ...
        data: {
          ...
          examLessonResults: exams.flatMap(
            (e) => e.attempts?.flatMap((a) => a.lessonResults || []) || [],
          ),
          examScores: exams.flatMap(
            (e) => e.attempts?.flatMap((a) => a.scores || []) || [],
          ),
          examNotifications: exams.flatMap((e) => e.notifications || []),

          // Message System
          messages: messageData[0],
          messageAttachments: messageData[0].flatMap(
            (m) => m.attachments || [],
          ),
          ...
          groupStudyPlans: groupData[0].flatMap((g) => g.groupPlans || []),
      ...
      console.log(
        `Total data types backed up: ${backupContent.metadata.dataTypes.length}`,
      );
    ...
    try {
      ...
      await this.prisma.$transaction(
        async (tx) => {
          ...
          await tx.groupTeacher.deleteMany({ where: { schoolId } });
          ...
          await tx.studentPerformanceSummary.deleteMany({
            where: { schoolId },
          });
          await tx.studyPlan.deleteMany({ where: { schoolId } });

          await tx.examNotification.deleteMany({
            where: { exam: { schoolId } },
          });
          ...
          if (data.school) {
            await tx.school.update({
              ...
              data: {
                ...
                isParentLoginActive: data.school.isParentLoginActive,
                studentLoginType: data.school.studentLoginType,
                autoCleanupEnabled: data.school.autoCleanupEnabled,
                cleanupMonthsToKeep: data.school.cleanupMonthsToKeep,
                lastCleanupAt: data.school.lastCleanupAt,
              },
            });
          }

          if (data.examCalendarSettings) {
            await tx.examCalendarSettings.create({
              data: {
                schoolId,
                showPublisher: data.examCalendarSettings.showPublisher,
                showBroughtBy: data.examCalendarSettings.showBroughtBy,
                showFee: data.examCalendarSettings.showFee,
                showParticipantCounts:
                  data.examCalendarSettings.showParticipantCounts,
                notifyDaysBefore: data.examCalendarSettings.notifyDaysBefore,
                autoPublishDaysAfter:
                  data.examCalendarSettings.autoPublishDaysAfter,
                defaultView: data.examCalendarSettings.defaultView,
                academicYearStart: data.examCalendarSettings.academicYearStart,
                createdAt: data.examCalendarSettings.createdAt,
                updatedAt: data.examCalendarSettings.updatedAt,
              },
            });
          }

          // 3. Restore Users (required for foreign keys)
          ...
          if (data.examAttempts?.length) {
            console.log(
              `Restoring ${data.examAttempts.length} exam attempts...`,
            );
          ...
          if (data.examLessonResults?.length) {
            console.log(
              `Restoring ${data.examLessonResults.length} lesson results...`,
            );
          ...
          if (data.examNotifications?.length) {
            console.log(
              `Restoring ${data.examNotifications.length} notifications...`,
            );
            ...
            await tx.messageSettings.create({
              data: {
                ...
                requireTeacherApproval:
                  data.messageSettings.requireTeacherApproval,
                enableEmailNotifications:
                  data.messageSettings.enableEmailNotifications,
                enablePushNotifications:
                  data.messageSettings.enablePushNotifications,
          ...
          if (data.mentorGroups?.length) {
            console.log(
              `Restoring ${data.mentorGroups.length} mentor groups...`,
            );
          ...
          if (data.groupStudyPlans?.length) {
            for (const groupPlan of data.groupStudyPlans) {
              await tx.groupStudyPlan.create({
                data: {
                  id: groupPlan.id,
                  groupId: groupPlan.groupId,
                  studyPlanId: groupPlan.studyPlanId,
                  assignedAt: groupPlan.assignedAt,
                },
              });
            }
          }
          ...
          if (data.achievements?.length) {
            console.log(
              `Restoring ${data.achievements.length} achievements...`,
            );
