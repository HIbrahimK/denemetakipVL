...
export interface ParsedExamRow {
  ...
  lessons: Record<
    string,
    { correct?: number; incorrect?: number; net?: number; point?: number }
  >;
...
export class ExcelParsingService {
   ...
   */
  private parseGradeAndSection(classText?: string): {
    grade: string;
    section: string;
  } {
  ...
  private detectFormat(worksheet: ExcelJS.Worksheet, examType?: string) {
    ...
    console.log(
      `[ExcelParsing] Format Algılanıyor. Sütun: ${colCount}, Hücre1: "${firstCell}"`,
    );
    ...
    if (colCount >= 30 && colCount <= 35)
      return { type: 'LGS', format: 'RAW', map: LGS_COLUMN_MAP };
    if (colCount >= 51 && colCount <= 55)
  ...
  private validateExamTypeMatch(detected: any, expected?: string) {
    if (expected && expected !== detected.type) {
      throw new Error(
        `HATA: ${expected} alanına ${detected.type} dosyası yüklenemez.`,
      );
    }
  }

  private parseByColumn(
    ...
    worksheet.eachRow((row, rowIndex) => {
      ...
      if (
        !studentNumber ||
        ['Genel', 'Ortalama', 'Liste'].some((k) => studentNumber.includes(k))
      )
        return;
      ...
      if (format.format === 'PROCESSED') {
        ...
        // B) PUANLAR
        if (map.scoreCols) {
          Object.entries(map.scoreCols).forEach(
            ([name, col]: [string, any]) => {
        ...
        if (map.rankStartCol) {
          ...
          Object.entries(map.rankGroups).forEach(
            ([groupName, startCol]: [string, any]) => {
              map.rankOrder.forEach((rankName, idx) => {
      ...
      // --- YÖNTEM 2: MAP TABANLI OKUMA (RAW / KURUM NET LİSTESİ) ---
        ...
        if (map.columns.lessons) {
          Object.entries(map.columns.lessons).forEach(
            ([name, cols]: [string, any]) => {
        ...
        if (map.columns.scores) {
          Object.entries(map.columns.scores).forEach(
            ([name, col]: [string, any]) => {
              rowData.scores[name] = Number(row.getCell(col).value) || 0;
            },
          );
        ...
        if (map.columns.ranks) {
          Object.entries(map.columns.ranks).forEach(
            ([name, col]: [string, any]) => {
