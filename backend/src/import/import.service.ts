...
export class ImportService {
            ...
            for (const row of rowsToProcess) {
                ...
                if (!student) {
                    ...
                    const studentUser = await tx.user.create({
                        data: {
                            email: `${studentNumber}.${schoolId}.s@denemetakip.com`, // slightly more unique email
                            password: '1234',
                            firstName,
                            lastName,
                            role: 'STUDENT',
                    ...
                    // Update Student Info (Name might have changed or was placeholder)
                    ...
                    // Update Student Record (Class and TC)
                ...
                // Clear previous results for this attempt to prevent duplication/stale data
    ...
    async validateImport(file: Buffer, examId: string, schoolId: string, examType: string): Promise<ParsedExamRow[]> {
        ...
        for (const row of rawRows) {
            ...
            // Ideally check DB for duplicates too if we want to warn, 
            // but for now we trust upsert logic or just warn on file duplicates.
        }
