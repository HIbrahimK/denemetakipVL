...
export class ImportService {
  ...
  async confirmImport(
    ...
    try {
      ...
      const rowsToProcess = data.filter(
        (row) => row.isValid && row.selected !== false,
      );
      ...
      await this.prisma.$transaction(
        async (tx) => {
          for (const row of rowsToProcess) {
            ...
            if (row.class) {
              const classText = (row.class as string).trim();
            ...
            await tx.examLessonResult.deleteMany({
              where: { attemptId: attempt.id },
            });

            if (row.lessons) {
              for (const [lessonName, stats] of Object.entries(row.lessons)) {
                const lessonStats = stats as {
                  correct: number;
                  incorrect: number;
                  net: number;
                  point: number;
            ...
            if (row.scores) {
              for (const [scoreType, scoreVal] of Object.entries(row.scores)) {
                ...
                if (examType === 'AYT') {
                  ...
                  rankClass =
                    row.ranks['Sınıf'] || row.ranks['Sınıf Derece'] || null;
                  rankSchool =
                    row.ranks['Okul'] ||
                    row.ranks['Kurum'] ||
                    row.ranks['Okul Derece'] ||
                    null;
                  rankDistrict = row.ranks['İlçe'] || null;
                  rankCity = row.ranks['İl'] || row.ranks['İl Derece'] || null;
                  rankGen =
                    row.ranks['Genel'] || row.ranks['Genel Derece'] || null;
                }
      ...
      for (const attemptId of attemptIds) {
        try {
          ...
          this.logger.warn(
            `Failed to check achievements for attempt ${attemptId}: ${error.message}`,
          );
        }
      }
      ...
      this.logger.error(
        `Import confirmation error: ${error.message}`,
        error.stack,
      );
      throw new BadRequestException(
        error.message || 'Veriler kaydedilirken bir hata oluştu.',
      );
    }
  }
  ...
  async validateImport(
    ...
    try {
      ...
      rawRows.forEach((r) => {
        if (r.studentNumber) {
          studentNumberCounts.set(
            r.studentNumber,
            (studentNumberCounts.get(r.studentNumber) || 0) + 1,
          );
        }
      });
      ...
      const existingStudentNumbers = new Set(
        existingAttempts.map((a) => a.student.studentNumber),
      );
      ...
      for (const row of rawRows) {
        ...
        if (existingStudentNumbers.has(studentNumber)) {
          ...
          if (!row.errorReason.includes('Sınava zaten kayıtlı')) {
            row.errorReason.push(
              'Bu öğrenci sınava zaten kayıtlı (veriler güncellenecek)',
            );
          }
          continue;
        }
        ...
        if (!systemStudentNumbers.has(studentNumber)) {
          ...
          if (!row.errorReason.includes('Sistemde kayıtlı değil')) {
            row.errorReason.push(
              'Sistemde kayıtlı değil (yeni öğrenci oluşturulacak)',
            );
          }
      ...
      this.logger.error(
        `Import validation error: ${error.message}`,
        error.stack,
      );
      throw new BadRequestException(
        error.message || 'Excel dosyası işlenirken bir hata oluştu.',
      );
    }
  }
  ...
  async handleFileUpload(
    ...
    const validatedData = await this.validateImport(
      file,
      examId,
      schoolId,
      examType,
