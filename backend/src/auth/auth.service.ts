...
export class AuthService {
  ...
  async loginSchool(loginDto: LoginDto) {
    ...
    if (
      !user ||
      !['SCHOOL_ADMIN', 'TEACHER', 'SUPER_ADMIN'].includes(user.role)
    ) {
  ...
  async register(
    ...
    if (isSchoolAdmin && registerDto.schoolId !== actor.schoolId) {
      throw new ForbiddenException(
        'Sadece kendi okulunuz için kullanıcı oluşturabilirsiniz',
      );
    }

    if (
      isSchoolAdmin &&
      !['TEACHER', 'STUDENT', 'PARENT'].includes(registerDto.role)
    ) {
