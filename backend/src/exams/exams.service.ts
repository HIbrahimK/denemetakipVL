...
export class ExamsService {
  ...
  async getExamStatistics(id: string, user?: any) {
    ...
    if (user) {
      ...
      if (user.role === 'STUDENT') {
        const hasAttempt = attempts.some(
          (attempt) => attempt.student.userId === user.id,
        );
        if (!hasAttempt) {
          throw new ForbiddenException('Access denied');
        }
    ...
    const branchStats: Record<
      string,
      { count: number; totalNet: number; totalScore: number }
    > = {};
    const lessonStats: Record<
      string,
      {
        count: number;
        totalNet: number;
    ...
    const finalLessonStats = Object.entries(lessonStats).map(
      ([name, stats]) => ({
        name,
    ...
    const finalBranchStats = Object.entries(branchStats).map(
      ([name, stats]) => ({
        name,
        avgNet: stats.count > 0 ? stats.totalNet / stats.count : 0,
    ...
    const studentList = attempts
      .map((a) => ({
        ...
        lessons: a.lessonResults.reduce(
          (acc, curr) => ({
            ...acc,
            [curr.lesson.name]: {
              net: curr.net,
  ...
  async uploadAnswerKey(examId: string, file: Express.Multer.File) {
    ...
    if (!allowedMimeTypes.includes(file.mimetype)) {
      throw new Error(
        'Invalid file type. Only PDF, JPG, JPEG, PNG, and Excel files are allowed.',
      );
    }
  ...
  async duplicateExam(examId: string, dto: DuplicateExamDto) {
    ...
    for (const gradeLevel of dto.gradeLevels) {
      ...
      if (newExam.scheduledDateTime && newExam.isPublished) {
        await this.createExamNotifications(
          newExam.id,
          newExam.scheduledDateTime,
        );
      }
  ...
  // Otomatik bildirim olu�Ytur
  private async createExamNotifications(
    examId: string,
