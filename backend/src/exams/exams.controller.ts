...
export class ExamsController {
  ...
  async getAnswerKey(
    ...
    try {
      const { filePath, contentType } =
        await this.examsService.getAnswerKeyFile(id, req.user);
      res.setHeader('Content-Type', contentType);
      return res.sendFile(filePath);
    } catch (error) {
      throw new HttpException(
        error.message || 'Dosya bulunamad�"±',
  ...
  @Post(':id/duplicate')
  @Roles('SCHOOL_ADMIN')
  duplicateExam(@Param('id') id: string, @Body() dto: DuplicateExamDto) {
