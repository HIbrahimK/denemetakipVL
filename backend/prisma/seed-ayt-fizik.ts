...
async function main() {
  ...
  // AYT Fizik Dersini oluştur veya güncelle
  let fizikSubject = await prisma.subject.findFirst({
    where: { name: 'Fizik', examType: ExamType.AYT },
  });

  if (!fizikSubject) {
    fizikSubject = await prisma.subject.create({
      data: {
        name: 'Fizik',
    ...
    console.log(`AYT Fizik dersi zaten mevcut: ${fizikSubject.id}`);
  }
  ...
  for (const konu of konular) {
    let parentTopic = await prisma.topic.findFirst({
      where: { name: konu.name, subjectId: fizikSubject.id, parentTopicId: null },
    });
    ...
    for (const child of konu.children) {
      const existingChild = await prisma.topic.findFirst({
        where: { name: child.name, subjectId: fizikSubject.id, parentTopicId: parentTopic.id },
      });
